...
console.log(idTime);
if (idText)
    idText.textContent = 'Texto: ' + (text);
if (idNumber)
    idNumber.textContent = 'Numero: ' + (number);
if (idBoolean)
    idBoolean.textContent = 'Booleano: ' + (boolean);
if (idTime) {
    const timeDate = (`${date.getDate()}/${date.getMonth()}/${date.getFullYear()}`);
    idTime.textContent = 'Fecha: ' + timeDate;
}
//Ejercicio 4
function converString(number) {
    return String(number);
}
const idCadString = document.getElementById('cadString');
if (idCadString)
    idCadString.textContent = `Numero convertido a cadena: ${converString(456)}`;
...
if (idSum)
    idSum.textContent = `Suma del array: ${sumArray(numbers)}`;
...
if (idName)
    idName.textContent = `Estudiante: ${study.name}`;
if (idAge)
    idAge.textContent = `Edad: ${study.age}`;
if (idCourse)
    idCourse.textContent = `Curso: ${study.course}`;
...
if (idDirection)
    idDirection.textContent = `Direccion: calle: ${direction.street}, ciudad: ${direction.city}, CP: ${direction.cp}`;
...
let saludo = person.saludar();
if (idInterface)
    idInterface.textContent = `${person.saludar()} `;
...
if (idClass)
    idClass.textContent = `${persona.mostrarDatos()}`;
...
class Caja {
    ...
    mostrar() {
        if (typeof this.value === "number")
            return `Contenido de cajaDeNumero: ${this.value}`;
        if (typeof this.value === "string")
            return `Contenido de cajaDeTexto: ${this.value}`;
    }
}
const caja1 = new Caja(42);
const caja2 = new Caja('Mensaje Secreto');
const idValueString = document.getElementById('valueString');
const idValueNumber = document.getElementById('valueNumber');
if (idValueString)
    idValueString.textContent = `${caja1.mostrar()}`;
if (idValueNumber)
    idValueNumber.textContent = `${caja2.mostrar()}`;
...
function identidad(value) {
    if (typeof value === "number")
        return `identidad del numero: ${value}`;
    if (typeof value === "string")
        return `identidad del texto: ${value}`;
}
...
if (idIdentityNumber)
    idIdentityNumber.textContent = `${identidad(123)}`;
if (idIdentityString)
    idIdentityString.textContent = `${identidad("hola")}`;
...
(function (Color) {
    ...
    Color[Color["Morado"] = 1] = "Morado";
    Color[Color["Rojo"] = 2] = "Rojo";
})(Color || (Color = {}));
const color = Color.Morado;
const enumColor = document.getElementById('enum');
if (enumColor)
    enumColor.textContent = `Color Favorito: ${Color[color]}`;
